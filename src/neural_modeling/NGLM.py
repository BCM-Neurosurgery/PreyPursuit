...
# class for formatting data and fitting neural glm models
class NGLM:
    ...
    # fit GAM with SVI training
    def fit(self, psth: List[np.ndarray], neuron_idx: int, **kwargs) -> SVIRunResult:
        ...
        if fit_type != "vi":
            raise NotImplementedError(
                "only variational inference fitting is currently supported"
            )
        if guide != "normal":
            raise NotImplementedError("only normal guide is currently supported")
        if optimizer != "scheduled":
            raise NotImplementedError(
                "only Adam scheduled decay optimizer is currently supported"
            )
        ...
        svi = SVI(
            prs_double_penalty,
            guide,
            chain(clip(10.0), adam(learning_rate_schedule)),
            loss=Trace_ELBO(),
        )

        # now run svi algo for model fitting per neuron
        fr_mat = psth[:, neuron_idx]
        self.y = fr_mat
    ...
    # sample posteriors from fitted model
    def sample_posteriors(
        self, n_samples: int = 4000, baseline: bool = False
    ) -> Dict[str, np.ndarray]:
        fit_res = self.fit_res if not baseline else self.noise_res
        guide = self.guide if not baseline else self.noise_guide

        posterior_samples = guide.sample_posterior(
            PRNGKey(1), fit_res.params, sample_shape=(n_samples,)
        )
        posterior_samples = {
            key: posterior_samples[key]
            for key in posterior_samples
            if key.startswith("beta_") or key.startswith("intercept")
        }
        self.posterior_samples = posterior_samples
        self.n_post_samples = n_samples
        return posterior_samples

    # summary stats for posterior sample
    def summarize_posterior(
        self, posterior_samples: Dict[str, jnp.ndarray], credible_interval: int = 90
        ...
        for keys in posterior_samples.keys():
            ...
            self.posterior_ci_lower[keys] = jnp.percentile(
                posterior_samples[keys], lower, axis=0
            )
            self.posterior_ci_upper[keys] = jnp.percentile(
                posterior_samples[keys], upper, axis=0
            )
    ...
    # get relevance of coefficients in GAM from posterior sample
    def coeff_relevance(
        self,
        ...
        for keys in posterior_samples.keys():
            self.coef_keep[keys] = np.logical_xor(
                ci_lower[keys] > 0, ci_upper[keys] < 0
            ).astype(int)

        return self.coef_keep

    def compute_idata(
        self, posterior_samples: Dict[str, jnp.ndarray]
    ) -> az.InferenceData:
        pointwise_log_likelihood = []
        basis_keys = [
            key
            for key in posterior_samples
            if key.startswith("beta_") and not key.startswith("beta_tensor_")
        ]
        tensor_keys = [
            key for key in posterior_samples if key.startswith("beta_tensor_")
        ]
        intercept_keys = [
            key for key in posterior_samples if key.startswith("intercept")
        ]
        ...
        for i in range(n_samples):
            ...
            for j, key in enumerate(basis_keys):
                if j == 0:
                    linear_pred = jnp.dot(
                        posterior_samples[key][i],
                        self.design_mat["bases"][j].transpose(),
                    )
                else:
                    linear_pred += jnp.dot(
                        posterior_samples[key][i],
            ...
            for j, key in enumerate(tensor_keys):
                linear_pred += jnp.dot(
                    posterior_samples[key][i],
                    self.design_mat["interaction_tensors"][j].transpose(),
                )
        ...
        idata = az.from_dict(
            log_likelihood={"log_likelihood": pointwise_log_likelihood}
        )
        return idata
