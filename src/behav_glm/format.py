...
def format_glm_inputs(switch_df, all_trial_df, session_df, wt_trials):
    input_rows = []
    for idx, tid in enumerate(all_trial_df['trial_id'].unique()):
        ...
        rel_speed1 = np.cumsum(trial_df['deltaspeedPrey1'] - np.mean(trial_df['deltaspeedPrey1'])).values
        rel_speed2 = np.cumsum(trial_df['deltaspeedPrey2'] - np.mean(trial_df['deltaspeedPrey2'])).values
        ...
        if len(trial_switches) > 0:
            for jdx, switch_row in trial_switches.iterrows():
                ...
                if switch_row.subtype == 1:
                    pursuit_dist = dist2[np.arange(switch_row.start_idx - 8, switch_row.start_idx - 1).astype(int)]
                    ignore_dist = dist1[np.arange(switch_row.start_idx - 8, switch_row.start_idx - 1).astype(int)]
                    pursuit_speed = rel_speed2[np.arange(switch_row.start_idx - 8, switch_row.start_idx - 1).astype(int)]
                    ignore_speed = rel_speed1[np.arange(switch_row.start_idx - 8, switch_row.start_idx - 1).astype(int)]
                else:
                    pursuit_dist = dist1[np.arange(switch_row.start_idx - 8, switch_row.start_idx - 1).astype(int)]
                    ignore_dist = dist2[np.arange(switch_row.start_idx - 8, switch_row.start_idx - 1).astype(int)]
                    pursuit_speed = rel_speed1[np.arange(switch_row.start_idx - 8, switch_row.start_idx - 1).astype(int)]
                    ignore_speed = rel_speed2[np.arange(switch_row.start_idx - 8, switch_row.start_idx - 1).astype(int)]
        ...
        # non-switch control trials - use median timepoint and median pursuit target for simplicity
            ...
            if median_target == 1:
                pursuit_dist = dist2[np.arange(median_timestep - 8, median_timestep - 1)]
                ignore_dist = dist1[np.arange(median_timestep - 8, median_timestep - 1)]
                pursuit_speed = rel_speed2[np.arange(median_timestep - 8, median_timestep - 1)]
                ignore_speed = rel_speed1[np.arange(median_timestep - 8, median_timestep - 1)]
            else:
                pursuit_dist = dist1[np.arange(median_timestep - 8, median_timestep - 1)]
                ignore_dist = dist2[np.arange(median_timestep - 8, median_timestep - 1)]
                pursuit_speed = rel_speed1[np.arange(median_timestep - 8, median_timestep - 1)]
                ignore_speed = rel_speed2[np.arange(median_timestep - 8, median_timestep - 1)]
            # now create row to add to df
            row = {
                'switch_subtype': 0,
...
def normalize_glm_inputs(glm_input_df):
    ...
    kin_cols = [col for col in glm_input_df.columns if (col.startswith('average')) or (col.startswith('slope'))]
    for col in kin_cols:
