...
def get_switch_types(
    ...
    # zscore windows
    windows_z = (wt_windows - np.mean(wt_windows, axis=1, keepdims=True)) / np.std(
        wt_windows, axis=1, keepdims=True
    )
    ...
    scores_pc1 = (split_scores[:, 0] - np.mean(split_scores[:, 0])) / np.std(
        split_scores[:, 0]
    )
    ...
    scores_pc2 = (split_scores[:, 1] - np.mean(split_scores[:, 1])) / np.std(
        split_scores[:, 1]
    )
    ...
    subscores_pc1 = (subscores_all[:, 0] - np.mean(subscores_all[:, 0])) / np.std(
        subscores_all[:, 0]
    )
...
def detect_switch_bounds(
    trial_wts: List[np.ndarray], switch_df: pd.DataFrame, window_size: int = 8
) -> pd.DataFrame:
    switch_starts = []
    switch_ends = []
    ...
    for _, row in switch_df.iterrows():
        ...
        try:
            ...
            if row.cross_idx - window <= 0:
                raise ValueError(
                    "crossing is too near the edge for change-point detection"
                )
            ...
            dd_wt = np.gradient(np.gradient(trial_wt[window_idcs].flatten())).reshape(
                -1, 1
            )
            ...
            average_bounds = (
                (bound_idx_rbf[[0, 2]] + bound_idx_l1[[0, 2]]) / 2
            ).astype(int)
...
def add_relative_reward(
    switch_df: pd.DataFrame, session_df: pd.DataFrame
) -> pd.DataFrame:
    rel_rewards = []
    for _, row in switch_df.iterrows():
