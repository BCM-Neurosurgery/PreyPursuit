...
class dPCA(BaseEstimator):
    ...
    def _optimize_regularization(self, X, trialX, center=True, lams="auto"):
        ...
        if np.argmin(totalscore) == 0 or np.argmin(totalscore) == len(totalscore) - 1:
            if self.debug > 0:
                print(
                    "Warning: Optimal regularization parameter lies at the \
    ...
    def _add_regularization_l1(self, Y, mYs, lam):
        """
        Applies L1 regularization to the data matrix and its marginalizations.

        Parameters:
            Y : np.ndarray
                Original data matrix of shape (n_features, n_samples).
            mYs : dict
                Marginalized data matrices.
            lam : float
                Regularization strength.

        Returns:
            regY : np.ndarray
                Regularized data matrix.
            regmYs : dict
                Regularized marginalized data matrices.
        """
        n_features, n_samples = Y.shape
        regmYs = {}

        # Apply L1 regularization for each marginalization
        for key, mY in mYs.items():
            reshaped_mY = mY.reshape((n_features, -1))
            reshaped_Y = Y.reshape((n_features, -1))

            # Use Lasso regression to find sparse projection weights
            lasso = Lasso(alpha=lam, fit_intercept=False, max_iter=1000)
            lasso.fit(
                reshaped_Y.T, reshaped_mY.T
            )  # Predict marginalized data using the original data

            # The coefficients are the regularized projection weights
            P_reg = lasso.coef_.T  # Shape (n_features, n_features)

            # Apply the weights to get the regularized marginalized data
            regmYs[key] = np.dot(P_reg, reshaped_Y)

        # Regularize the original data matrix (optional, depending on dPCA needs)
        regY = np.dot(P_reg, reshaped_Y)
    ...
    def _fit(self, X, trialX=None, mXs=None, center=True, SVD=None, optimize=True):
        ...
        if self.regularizer > 0:
            ...
            regX, regmXs, pregX = self._add_regularization_l1(X, mXs, self.regularizer)
        else:
    ...
    def train_test_split(self, X, trialX, N_samples=None, sample_ax=0):
        ...
        if sample_ax != 0:
            raise NotImplementedError("The sample axis needs to come first.")
