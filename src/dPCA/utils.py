...
def smooth_fr(fr: np.ndarray, sampling_rate: int = 60, window_size_ms: int = 60):
    time_per_sample = 1 / sampling_rate
    smoothing_window_samples = window_size_ms / 1000 / time_per_sample
    sigma = smoothing_window_samples / 2.355

    fr_smooth = gaussian_filter1d(fr, sigma=sigma, axis=1)
    return fr_smooth
