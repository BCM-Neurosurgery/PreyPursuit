...
def format_trial(design_mat: pd.DataFrame, trial_id: int, dt: float=1/60, decay_term: float=0, n_rbfs: int=30):
    state_matrix = np.array([[1, 0, dt, 0],
                             [0, 1, 0, dt],
    ...
    trial_data['prey2_pos'] = np.vstack((trial_mat.prey2Xpos, trial_mat.prey2Ypos)).transpose()
    trial_data['prey2_pos'] = jnp.array(trial_data['prey2_pos'])

    # add prey 1 velocity data
    trial_data['prey1_vel'] = np.vstack((trial_mat.prey1Xvel, trial_mat.prey1Yvel)).transpose()
    trial_data['prey1_vel'] = jnp.array(trial_data['prey1_vel'])

    # add prey 2 velocity data
    trial_data['prey2_vel'] = np.vstack((trial_mat.prey2Xvel, trial_mat.prey2Yvel)).transpose()
    trial_data['prey2_vel'] = jnp.array(trial_data['prey2_vel'])

    # add player acceleration data (control signal)
    trial_data['player_accel'] = np.vstack((trial_mat.selfXaccel, trial_mat.selfYaccel)).transpose()
    trial_data['player_accel'] = jnp.array(trial_data['player_accel'])

    # add prey 1 acceleration data
    trial_data['prey1_accel'] = np.vstack((trial_mat.prey1Xaccel, trial_mat.prey1Yaccel)).transpose()
    trial_data['prey1_accel'] = jnp.array(trial_data['prey1_accel'])

    # add prey 2 acceleration data
    trial_data['prey2_accel'] = np.vstack((trial_mat.prey2Xaccel, trial_mat.prey2Yaccel)).transpose()
    trial_data['prey2_accel'] = jnp.array(trial_data['prey2_accel'])

    # get normalized timeline
    timeline = np.linspace(0, len(trial_data['prey1_accel']), len(trial_data['prey1_accel']))
