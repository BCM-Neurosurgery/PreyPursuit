...
class NLLLoss:
    def __init__(
        ...
        prior_std: Dict[str, int] = {"weights": 10, "widths": 2, "gains": 5},
    ):
    ...
    def _create_loss(
        ...
        use_gmf_prior: bool,
        lambda_reg: float,
        prior_std: Dict[str, int],
        likelihood_only: bool,
    ) -> Callable[[np.ndarray, Dict[str, Any]], float]:
        # pick controller error
        controller_err = CONTROL_ERROR[control_type]

        @jit
        def loss_func(params: np.ndarray, inputs: Dict[str, Any]) -> float:
            ...
            def _control_step(k, val):
                ...
                if control_type == "p":
                    x_next = (
                        inputs["state_matrix"] @ x[k]
                        + (inputs["control_matrix"] @ u).flatten()
                    )
                    x = x.at[k + 1].set(x_next)
                    u_out = u_out.at[k].set(u.flatten())
                else:
                    x_next = (
                        inputs["state_matrix"] @ x[k] + inputs["control_matrix"] @ u
                    )
                    x = x.at[k + 1].set(x_next)
            ...
            x, u_out, err_int_pos1, err_int_pos2 = lax.fori_loop(
                0, n_steps, _control_step, (x, u_out, err_int_pos1, err_int_pos2)
            )
            ...
            prior_gains = -0.5 * (
                jnp.sum((L1 / prior_std["gains"]) ** 2)
                + jnp.sum((L2 / prior_std["gains"]) ** 2)
            )

            loss = (
                -log_likelihood
                - lambda_reg * gmf_prior
                - 0 * prior_weights
                - prior_widths
                - prior_gains
            )
