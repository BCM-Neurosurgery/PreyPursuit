...
class Controller:
    ...
    def fit(self) -> Tuple[Tuple, np.ndarray, float]:
        initial_weights = np.zeros(self.num_rbfs) + int(
            self.randomize_weights
        ) * np.ones_like(np.zeros(self.num_rbfs) * np.random.randn(self.num_rbfs))
        ...
        def optimizer_gradient(params_flat: np.ndarray) -> np.ndarray:
        ...
        def optimizer_hessian(params_flat: np.ndarray) -> np.ndarray:
        ...
        L1 = best_params_flat[
            (self.num_rbfs + 1) : (self.num_rbfs + self.gain_size + 1)
        ]
        L2 = best_params_flat[
            (self.num_rbfs + self.gain_size + 1) : (
                self.num_rbfs + 2 * self.gain_size * 2 + 1
            )
        ]
    ...
    def simulate_trajectory(self) -> np.ndarray:
        try:
        ...
        shift_matrix = generate_shift_matrix(
            self.inputs["timeline"], self.inputs["centers"], widths, rbf_weights
        )

        trajectory = simulate(
            self.inputs, L1_fit, L2_fit, self.control_type, shift_matrix=shift_matrix
        )
        return trajectory

    def calc_covariance_matrix(self) -> np.ndarray:
        try:
            best_params_flat = self.best_params_flat
        except AttributeError:
            raise NotFittedError(
                "have to fit model before calculating covariance matrix!"
            )
        ...
        prior_hessian = np.block(
            [
                [
                    H_weights,
                    np.zeros((self.num_rbfs, 1)),
                    np.zeros((self.num_rbfs, num_gains)),
                ],  # Weights
                [
                    np.zeros((1, self.num_rbfs)),
                    H_widths,
                    np.zeros((1, num_gains)),
                ],  # Widths
                [
                    np.zeros((num_gains, self.num_rbfs)),
                    np.zeros((num_gains, 1)),
                    H_gains,
                ],  # Gains
            ]
        )
        ...
        # covariance
        cov = np.linalg.inv(
            hessian + prior_hessian + regularization * np.eye(hessian.shape[0])
        )
    ...
    def compute_elbo(self) -> float:
        ...
        posterior_logdet = (
            0.5 * np.linalg.slogdet(cov_matrix)[1]
        )  # log(|Sigma_posterior|)
        normalization_constant = -0.5 * num_params * np.log(2 * np.pi)

        # Draw posterior samples
        samples = np.random.multivariate_normal(
            mean=self.best_params_flat, cov=cov_matrix, size=self.elbo_samples
        )
        ...
        for sample in samples:
            ...
            log_prior_weights = -0.5 * (
                weights @ S_x @ weights.T
            )  # GMF prior for weights
            log_prior_widths = -0.5 * np.sum(
                (widths / self.prior_std["widths"]) ** 2
            )  # Gaussian prior for widths
            log_prior_gains = -0.5 * np.sum(
                (gains / self.prior_std["gains"]) ** 2
            )  # Gaussian prior for gains
    ...
    def get_bayesian_trajectory(self, num_samples: int = 1000) -> np.ndarray:
        try:
            cov_matrix = self.cov_matrix
            best_params_flat = self.best_params_flat
            inputs = self.inputs
            ...
            raise NotFittedError(
                "have to calculate covariance matrix before bayesian trajectory!"
            )

        # Sample from posterior
        samples = np.random.multivariate_normal(
            mean=best_params_flat, cov=cov_matrix, size=num_samples
        )
        ...
        for sample in samples:
            ...
            traj = generate_shift_matrix(
                inputs["timeline"], inputs["centers"], widths, weights
            )
