...
def set_larger_prey_first(
    design_df: pd.DataFrame, behav_df: pd.DataFrame
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    flip_mask = (behav_df["preys_num"] == 2) & (
        behav_df["prey2_val"] > behav_df["prey1_val"]
    )
...
def add_relative_reward(
    design_df: pd.DataFrame, behav_df: pd.DataFrame, n_prey: int = 2
) -> pd.DataFrame:
