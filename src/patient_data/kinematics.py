...
def build_kinematics(
    ...
    def _get_col(col, idx=None):
        ...
        if v.dtype == object:
            if idx is None:
                return v.apply(
                    lambda a: np.nan if not hasattr(a, "shape") else (a * rescale)
                )
            return v.apply(
                lambda a: np.nan
                if not hasattr(a, "shape") or a.shape[0] <= idx
                else a[idx] * rescale
            )
        return v * rescale
    ...
    std_len = np.std(trial_lens)
    out["rel_samples"] = (out["time_samples"] - median_len) / std_len

    return out.reset_index(drop=True)


def add_kinematic_features(df: pd.DataFrame, n_prey: int = 2) -> pd.DataFrame:
    # distances
    ...
    if n_prey == 2:
        ...
        denom = d1 + d2
        reldist = np.divide(
            d1 - d2, denom, out=np.zeros_like(denom, dtype=float), where=(denom != 0)
        )
        df["distPrey2"] = d2
    ...
    if n_prey == 2:
        ...
        denom = s1 + s2
        rs = np.divide(
            s1 - s2, denom, out=np.zeros_like(denom, dtype=float), where=(denom != 0)
        )
