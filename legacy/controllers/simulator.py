...
def controller_sim_p(tdat,shiftype,L1,L2,A=None,B=None,gpscaler=3,assignment='soft', empirical_wt=None):
    '''
    ...
    if shiftype == 1:
        ...
        if empirical_wt is None:
            raise ValueError("shiftype==7 requires empirical_wt array.")
        w = np.asarray(empirical_wt).astype(float).flatten()
        w = np.clip(w, 0.0, 1.0)
        ...
        for k in range(T):
            ...
            u = 0.0
            if use_state_drive:
                d1 = np.linalg.norm(player_pos[k, :] - pry1_pos[k, :])
                d2 = np.linalg.norm(player_pos[k, :] - pry2_pos[k, :])
...
def controller_sim_pv(tdat,shiftype,L1,L2,A=None,B=None,gpscaler=3,assignment='soft', empirical_wt=None):
    '''
...
def controller_sim_pvi(tdat, shiftype, L1, L2, A=None, B=None,gpscaler=3,dt=1.0/60.0,Ie_min=-10.0,Ie_max=10.0,assignment='soft', empirical_wt=None):
    '''
...
def controller_sim_pvif(tdat,shiftype,  L1, L2, A=None, B=None, gpscaler=3,dt=1.0/60.0,Ie_min=-10.0,Ie_max=10.0,assignment='soft', empirical_wt=None):
    '''
    Simulats controllers that track using positiion and velocity and positon error integral errors
    :param player_pos:
    :param shiftype:
...
def controller_sim_pif(tdat,shiftype,  L1, L2, A=None, B=None, gpscaler=3,dt=1.0/60.0,Ie_min=-10.0,Ie_max=10.0,assignment='soft', empirical_wt=None):
    '''
    Simulats controllers that track using positiion and velocity and positon error integral errors
    :param player_pos:
    :param shiftype:
...
def controller_sim_pf(tdat, shiftype,  L1, L2, A=None, B=None ,gpscaler=3,dt=1.0/60.0,assignment='soft', empirical_wt=None):
    '''
    Simulats controllers that track using positiion and velocity and positon error integral errors
    :param player_pos:
    :param shiftype:
...
def controller_sim_pvf(tdat, shiftype,  L1, L2, A=None, B=None ,gpscaler=3,dt=1.0/60.0 ,assignment='soft', empirical_wt=None):
    '''
    Simulats controllers that track using positiion and velocity and positon error integral errors
    :param player_pos:
    :param shiftype:
...
def controller_sim_p_slack(tdat,shiftype, L1, L2, A=None, B=None, gpscaler=3, alpha=10):
    ...
    if shiftype == 1:
        ...
        if w.size >= T:
            ...
            w = np.pad(w, (0, T - w.size), mode='edge')
        w = np.clip(w, 0.0, 1.0)
        shift = np.vstack((w, 1.0 - w))
