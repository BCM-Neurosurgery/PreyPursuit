                ...
                for gpscalidx, gpscaler in enumerate(cfgparams['gpscaler']):
                    ...
                    for _,trial in enumerate(trialidx):
                        for restart in range(cfgparams['restarts']):
                            ...
                            if cfgparams['gains_mode'] == 'random':
                                L1, L2 = ut.generate_sim_gains(cfgparams['ngains'][modidx], cfgparams['maxgain'])
                            elif cfgparams['gains_mode'] == 'random_extended':
                                L1, L2 = ut.generate_sim_gains_extended(cfgparams['ngains'][modidx])
                            elif cfgparams['gains_mode'] == 'empirical':
                                L1, L2 = ut.get_empirical_gains_for_model(
                                    modname, cfgparams['base_path']
                                )
                            else:
                                raise ValueError(f"Unknown gains_mode: {cfgparams['gains_mode']}")
                            
                            L1 = L1 * 3.0
                            L2 = L2 * 3.0

                            if cfgparams['wtsim_mode'] == 'empirical':
                                empirical_wt = ut.get_empirical_sim_switch(modname, cfgparams['wt_path'], tdat)
                            else:
                                empirical_wt = None 
                            ...
                            #Simulate testdata
                            if modname == 'p':
                                outputs = sim.controller_sim_p(tdat, cfgparams['shiftype'], L1, L2, A=None, B=None, gpscaler=gpscaler, assignment='soft', empirical_wt=empirical_wt)
                            elif modname == 'pv':
                                outputs = sim.controller_sim_pv(tdat, cfgparams['shiftype'], L1, L2, A=None, B=None, gpscaler=gpscaler, assignment='soft', empirical_wt=empirical_wt)
                            elif modname == 'pf':
                                outputs = sim.controller_sim_pf(tdat, cfgparams['shiftype'], L1, L2, A=None, B=None, gpscaler=gpscaler, assignment='soft', empirical_wt=empirical_wt)
                            elif modname == 'pvi':
                                outputs = sim.controller_sim_pvi(tdat, cfgparams['shiftype'], L1, L2, A=None, B=None, gpscaler=gpscaler, assignment='soft', empirical_wt=empirical_wt)
                            elif modname == 'pif':
                                outputs = sim.controller_sim_pif(tdat, cfgparams['shiftype'], L1, L2, A=None, B=None, gpscaler=gpscaler, assignment='soft', empirical_wt=empirical_wt)
                            elif modname == 'pvf':
                                outputs = sim.controller_sim_pvf(tdat, cfgparams['shiftype'], L1, L2, A=None, B=None, gpscaler=gpscaler, assignment='soft', empirical_wt=empirical_wt)
                            elif modname == 'pvif':
                                outputs = sim.controller_sim_pvif(tdat, cfgparams['shiftype'], L1, L2, A=None, B=None, gpscaler=gpscaler, assignment='soft', empirical_wt=empirical_wt)
                            ...
                            gaincorr_t1 = np.corrcoef(L1_fit, L1)[0, 1]
                            gaincorr_t2 = np.corrcoef(L2_fit, L2)[0, 1]
                            gaincorr_all = np.corrcoef(np.concatenate([L1_fit, L2_fit]), np.concatenate([L1, L2]))[0, 1]
                            ...
                            new_row['traj_real'] = outputs['x']      # full true trajectory
                            new_row['traj_fit'] = output_pred['x']   # reconstructed trajectory
                            new_row['u_real'] = outputs['uout']      # control signals (optional)
                            new_row['u_fit'] = output_pred['uout']   # predicted control (optional)
                            ...
                            new_row['gaincorr_t1'] = gaincorr_t1
                            new_row['gaincorr_t2'] = gaincorr_t2
                            new_row['gaincorr_all'] = gaincorr_all
...
def process_simulation_task(args):
    ...
    # generate gains
    L1, L2 = ut.generate_sim_gains(len(modname), cfgparams['maxgain'])
